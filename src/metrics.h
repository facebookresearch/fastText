...
struct Metrics {
  ...
  double precision() const {
    return numTruePositives / double(numPredictions);
  }
  double recall() const {
    return numTruePositives / double(numLabels);
  }
  double f1Score() const {
    return 2 * numTruePositives / double(numPredictions + numLabels);
  }
};
...
class LabelMetricsAccumulator : public MetricsAccumulator {
 public:
  void log(
      const std::vector<int32_t>& labels,
      const std::vector<std::pair<real, int32_t>>& predictions) override;
